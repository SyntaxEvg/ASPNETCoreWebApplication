...
namespace EmployeeService.Controllers
{
    ...
    public class AuthenticateController : ControllerBase
    {
        ...
        private readonly IValidator<AccountDTOREG> _registr;
        ...
        public AuthenticateController(
            ...
            IValidator<AccountDTOREG> registr)
        {
        ...
        public IActionResult Registration([FromBody] AccountDTOREG authenticationRequest)
        {
            ValidationResult validationResult = _registr.Validate(authenticationRequest);

            if (!validationResult.IsValid)
                return BadRequest(validationResult.ToDictionary());

            var authenticationResponse = _authenticateService.Registration(authenticationRequest);
            if (authenticationResponse.Status == AuthenticationStatus.UnsuccessfulRegistration)
            {
                return BadRequest(validationResult.ToDictionary());
            }
            authenticationResponse.Message = "successful registration";
